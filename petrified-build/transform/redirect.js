...
module.exports = function post({ source, options, ...rest })
{
    ...
    if (fs.tstat(source) === "directory")
    {
        ...
        return  <redirects source = { file }>
                    <tree { ...{ source, ...rest, transforms, destination } }/>
                </redirects>;
    }
    console.log("YES!");
...
function redirects({ source, children })
{
    const { frontmatter } = children.find(child => child.source === source);
    const redirects = frontmatter["redirect-from"] || frontmatter.redirect_from;
